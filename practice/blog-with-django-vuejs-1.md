---
description: '목표: 블로그 사이트 만들기 예제를 통해 django와 vuejs를 학습합니다.'
---

# Django + Vuejs로 블로그 사이트 만들기

이번에는 아래의 내용들을 차례대로 진행해보겠습니다.

1. 프로젝트 생성
2. 기본 블로그 앱 만들기
3. 포스트 CRUD 작성 및 연결하기
4. vuejs로 블로그 view 분리하기
...
4.  템플릿과 정적파일 디렉토리를 posts 앱의 하위에 아래와 같이 생성합니다.
    ...
    {% endraw %}"> <!-- 이렇게 하면 앱 하위에 static이라는 디렉토리 내에서 index.css라는 파일을 자동으로 찾아서 헤더에 삽입해줍니다. -->
...
5.  다시 [http://localhost:8000/posts/](http://localhost:8000/posts/)에 접속해보면 CSS가 적상적으로 적용된 페이지가 보입니다. 하지만, 여전히 포스트 내용을 HTML에 바인딩 하지는 못했습니다. 이번에는 포스트 목록을 불러와서 HTML에 바인딩해주는 작업을 해보겠습니다. 포스트 목록은 임시로 fake api를 호출해서 가져올 예정입니다.
    ...
    ```markup
     ...
     <div class="row">
         <div class="leftcolumn">
         <!-- view 템플릿 내에서 파이썬 문장을 실행할때는 아래처럼 {% raw %}
    {% ... %} 로 작성합니다. -->
         {% for post in posts %}
             <div class="card">
...
3. 생성한 데이터베이스에 다시 접속하여 정상적으로 생성되었는지 확인합니다. (1번 과정을 반복하여 아래 정보 입력)
   * Host: localhost
   * Port: 5432
   * Database: blog\_basic
   * username: blog\_admin
   * password: password
...
7.  모델을 작성했으면, 모델의 스키마를 실제 데이터베이스의 테이블 스키마와 동기화 해주는 작업이 필요합니다. 동기화는 migrate라는 작업으로 이루어지며, migration 파일을 필요로 합니다. 이는 아래와 같습니다.
    ...
    실제로 데이터베이스의 스키마에 반영되었는지 dbeaver를 통해 확인해봅니다. (blog\_basic 우클릭 > Refresh 클릭 > Schemas > public > Tables > posts\_post > Columns)
...
12. 관리자 화면에서는 정의된 모델에 값들을 추가하여 생성할 수 있습니다. 포스트 모델 생성 화면([http://localhost:8000/admin/posts/post/add/](http://localhost:8000/admin/posts/post/add/))에 접속하여 아래와 같이 데이터를 생성해봅니다.
    ...
    * Image url: [https://picsum.photos/id/1000/1200/300](https://picsum.photos/id/1000/1200/300)
    * Created at: 변경 X
    * Published at: 변경 X
